...
**It is perfectly fine if you want to draw like this**
...
[**Diagrams you can find online**]{style="color:royalblue"}

These are the UML class diagrams that you can find online if you Google "UML class diagrams"

![](./figures/07-relationships-between-classes.png)

![](./figures/class-diagram-relationship.png)

[**Diagram using `mermaid` in Quarto.**]{style="color:royalblue"}

To see the source code for this diagram, please have a look at `chap_17.qmd` file. This is the source code for this part:
...
- **Example**: A `Car` class realizing a the abstract class `Vehicle` .
...
In paper exam, we write `*ClassName*` with the double asterisks surrouding the `ClassName`.
:::
...
A `Car` has $4$ `Tire`s and $1$ `Engine`:
...
A `Library` can have many `Book`s and many `CD`s
...
classDiagram
    ...
    Library "1" o-- "*" CD : contains
```

One `Doctor` can have/examine many `Patient`s and one `Patient` can visit many `Doctor`s
